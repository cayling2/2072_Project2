...
ALL THE CODE ABOVE THIS POINT WAS COPY AND PASTED FROM PROJECT 1
...
#With help from Ceceilagh Pitstick, I realized that I made a new column in part 2 called new_sex and did not change the original sex column which would not run this chunk. So I manually changed the sex column and it ran. 

pData(scanAnnot)$sex[pData(scanAnnot)$scanID == 325] <- "M"
pData(scanAnnot)$sex[pData(scanAnnot)$scanID == 326] <- "M"
...
-   Summarize the pedigree structures you observe in the data.
    ...
    In the data I observed 16 reportedly unrelated samples showing up as related and 17 reportedly related samples showing up as unrelated. I also observed a discrepancy of paternity in family 58 and what looked to be a swapped sample in families 1334 and 1344 which was retified by swapping the children in the pedigrees to match the paternaty determined by genotyping.

-   Describe what discrepancies you observe in the initial kinship vs IBS0 plot.

    In the initial kinship vs ISB0 plot there were 3 distinct zones, that all had their own general color. The issue is that the parent-offspring zone (light blue) had some point labeled unrelated and there were some points in the unrelated zone that were labeled parent-offspring.

-   Explain exactly the kinds of swaps/discrepancies that were found, which IDs were involved, and exactly how you "fixed" them.

    The discrepancies found were in families 58, 1334, and 1344. In family 58, there was a case of non-paternity in which the labeled father (200105428) was not actually related to the child (200122151). This was fixed by changing the child's father to 0 and making the reported father his own family, which keeps the data, but helps with the relatedness calculation. In families 1334 and 1344 there seemed to be a sample swap where the children are related to the other family's parents. This was rectified by swaping the children (1334 scanID: 297 1344 scanID:298 ) to the correct mother(1334:200019634, 1344:200005043) and father(1334:200118596, 1344:200116780).
...
-   How would you recommend following up on these discrepancies "in real life" (instead of just assuming we know what happened and manually changing the data, which is a *bad idea*)?
    ...
    In real life, I would contact the PI and the lab techs to see if there is a logical explination for the discrepancies. I would also want to look at the data collection and any notes about that. If there was no good explanation for the discrepancies I would let the PI choose what to do, but would lean towards dropping the samples with the discrepancies.
...
```{r part4_report_relatedness}

#   <Your code goes here>

scanAnnot$relationship.changes <- NA

pData(scanAnnot)$relationship.changes[pData(scanAnnot)$family == "58"] <- "Changed father to 0 to account for non paternity"
pData(scanAnnot)$relationship.changes[pData(scanAnnot)$family == "58-2"] <- "Changed the original father of family 58 to a new family due to non paternity"
pData(scanAnnot)$relationship.changes[pData(scanAnnot)$family == "1334"] <- "swapped child with family 1344"
pData(scanAnnot)$relationship.changes[pData(scanAnnot)$family == "1344"] <- "swapped child with family 1334"

varMetadata(scanAnnot)["relationship.changes", "labelDescription"] <-
  "Updated pedigree information based on relatedness calculations"
...
-   How many SNPs were used to calculate PCs?
    ...
    There were 179 SNPs used to calculate PCs. The PCA data has a list of SNP IDs which is 179 in length.
...
-   What information does the first PC appear to convey?
    ...
    The first PC conveys that there is clear separation between the other PCs (2-4) and clear distinction between the races.

```{r part5_report}

#   <Your code goes here>

#make a pairwise scatterplot of the first 4 PCs
#Dif plotting symbols for the dif pops - race in scanAnnot
...
#new column for SNP IDs and their races
data$samp <- pca$sample.id

#looked at the scanAnnot data to determine this threshold
data$race[data$samp <= 328] <- "CEU"
data$race[data$samp > 328] <- "YRI"

ggpairs(data, columns = 1:4, ggplot2::aes(color = race), columnLabels = c("PCA1", "PCA2", "PCA3", "PCA4"))

#Make a parallel coordinate plot for the first 4 plots as well, again distinguishing the plotted lines according to the "race" variable.
ggparcoord(data, columns = 1:4, groupColumn = 34, showPoints = TRUE, alphaLines = 0.5)

#https://r-graph-gallery.com/parallel-plot-ggally.html
#This link was shared to me by Dr. Chernus and was very helpful for figuring out how to plot the coorinate plot, and when I typed the pa the ggpaired came up and I used it. I also double checked with Aakash about the ggpairs function. 
```
...
-   How many SNPs are discordant in at least one duplicate-pair?
    ...
    There are 34 SNPs that are discoordant in at least one duplicate pair.

-   What's the maximum number of discordances seen for any SNP?

    The maximum discordances for any SNP seen is 2.
...
-   You may find it helpful to call the function duplicateDiscordanceProbability(npair=34). Read the documentation for this function.
    ...
    Since the max amount of discordance by SNP is 2 and the number of pairs is 34 it can be assumed that the filtering threshold is 1 probe based on the slides in the lecture.

-   Of the SNPs pass that passed the missing.n2 filter (missing.n2 \< 0.05), how many SNPs pass/fail the discordance threshold?

    Of the SNPs that passed the missing.n2 \< 0.05, 3,095 passed the threshold of an n2 of \< 0.05 and a discordance of 0/1 based of duplicate pair data.
...
max(temp_snp$discordant, na.rm = TRUE)

#median and max = summary?
summary(temp_snp$discordant)


duplicateDiscordanceProbability(npair=34)

#missing.n2 < 0.05 and dups passing
temp_snp$discordant[temp_snp$missing.n2 < 0.05] %>% table()

#make histogram
#Of the SNPs with missing.n2 < 0.05, make a histogram of the discordance rate
temp_snp %>% 
  filter(temp_snp$missing.n2 < 0.05) %>% 
  ggplot() +
  geom_histogram(aes(x = discordant))
...
pData(snpAnnot)$failed.dup.disc[pData(snpAnnot)$discordant <= 1] <- "pass"
pData(snpAnnot)$failed.dup.disc[pData(snpAnnot)$discordant > 1] <- "fail"
pData(snpAnnot)$failed.dup.disc[pData(snpAnnot)$missing.n2 >= 0.05] <- "untested"

varMetadata(snpAnnot)["failed.dup.disc", "labelDescription"] <-
  "Updated discordance information with pass/fail depending on probes and untested if the n2 missingness was > 0.05"
```
...
-   How do you choose what subjects to include for HWE testing? How many were used here?
    ...
    To choose the subjects to include for HWE we need to make sure that they are unrealted in the pedigrees (founders) otherwise the relatedness will skew the data. In this data set 2,539 samples were used (the hwe.0 dataframe that was created has 2,539 rows).

-   Interpret the Q-Q plots. Do any SNPs appear to be strongly out of HWE? Which SNPs? What are the lowest p-values?

    In the QQ-plots the autosomal ones are the better of the 4, but still stray from the line indicating normality. The X-chromosome, espcially the onle labeled all, stray far from the normality line and indicate to be strongly out of HWE. See out_of_hwe for the SNPIDs. The lowest p-value is
...
-   What p-value will you use for filtering our SNPs with low HWE? Of the SNPs with missing.n2 \< 0.05, how many will be retained/removed?
    ...
    I will use a p-value of 0.05 since that is generally the accepted p-value in the field for most things. If you take the missing.n2 from question 6 and how many SNPs passed that and how many SNPs were in the hwe.0 data frame. 556 SNPs were removed from the data.
...
#SNPs out of HWE
out_of_hwe <- hwe.0$snpID[hwe.0$pval > 0.05]

#lowest p-val
min(hwe.0$pval, na.rm = TRUE)

#For the autosomal SNPs, make a plot of -log10(p-value) vs MAF. Make another such plot for the X chromosome.

maf <- hwe.0$MAF[is.element(hwe.0$chr, 1:22)]
length(maf)
...
maf.x <- hwe.0$MAF[is.element(hwe.0$chr, 23)]
length(pval.x)
maf.x <- maf.x[!is.na(pval.x)]
length(maf.x)

#plot for autosomes
ggplot()+
  geom_point(aes(x = maf, y = -log10(pval)))

#plot for x values
ggplot()+
  geom_point(aes(x = maf.x, y = -log10(pval.x)))
...
pData(snpAnnot)$hwe[pData(snpAnnot)$snpID == hwe$snpID] <- hwe$pval

varMetadata(snpAnnot)["hwe", "labelDescription"] <-
  "Updated the p-values from the HWE test"
```

Me, Vanessa Hong, and Ceceilagh Pitstick briefly went over our answers after completing the project.
